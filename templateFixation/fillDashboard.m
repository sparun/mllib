function lines = fillDashboard(TrialData, TrialRecord)

% DISPLAY the editable parameter values on the dashboard
tempVars         = TrialData.VariableChanges;
tempUser         = TrialRecord.User;
bufferSize       = 50;
expectedResponse = cell2mat(tempUser.expectedResponse);
responseCorrect  = cell2mat(tempUser.responseCorrect);
...
% JUICE consumption
totalJuiceConsumed = nansum(cell2mat(tempUser.juiceConsumed));

% DISPLAY strings
lines{1,1}  = sprintf('Good Pause =  %04d ms  |  Bad Pause = %04d ms | Reward = ml/click =  %1.2f ml | Total Juice = %3.1f ml',...
    tempVars.goodPause, tempVars.badPause, tempVars.rewardMlPerClick, totalJuiceConsumed);
